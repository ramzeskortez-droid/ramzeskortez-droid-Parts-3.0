...
Объекты проходят через следующие статусы:
1.  **OPEN (Заказ):** Виден всем поставщикам в ленте "Новые".
2.  **PROCESSED (Обработан):** Флаг `processed = 'Y'`. Заказ скрывается у поставщиков (если они не дали цену) и становится доступен клиенту во вкладке "В обработке".
3.  **READY_TO_BUY (Покупка):** Флаг `readyToBuy = 'Y'`. Клиент подтвердил выбор. Заказ уходит в архив.
4.  **CLOSED (Закрыт):** Финальное состояние. Запись заморожена.
