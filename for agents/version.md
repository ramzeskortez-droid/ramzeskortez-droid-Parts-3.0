...
## 1.9.34
- **Admin Interface:**
  - **Дизайн:** Реализован "Pop-out" эффект для развернутого заказа, аналогичный интерфейсу поставщика. При клике заказ визуально отделяется от списка (тень, синяя рамка слева, отступы).
  - **Grid:** Внедрена единая константа `GRID_COLS` для управления шириной колонок, что гарантирует идеальное выравнивание заголовков и строк в любых состояниях.
  - **Структура:** Удален `React.Fragment`, каждый заказ теперь обернут в `div`, который меняет свои классы в зависимости от состояния `isExpanded`.
- **Файлы:** `components/AdminInterface.tsx`.

## 1.9.33
- **Admin Interface:**
  - **UI:** Строка поиска восстановлена в виде большого блока над вкладками (дизайн унифицирован с интерфейсом поставщика).
  - **Layout:** Кнопка обновления (`RefreshCw`) перенесена в строку вкладок (справа).
  - **Header:** Убраны лишние элементы из шапки, оставлен только заголовок и кнопка истории.
- **Файлы:** `components/AdminInterface.tsx`.

## 1.9.32
- **Admin Interface:**
  - **Сортировка:** Исправлена сортировка по колонке "Год" (теперь сортирует по `car.year`, а не по дате).
  - **Архив:** Исправлена сортировка по колонке "СТАТУС". Теперь она учитывает логический вес статуса (Куплено > Готово > Отказ > Закрыт).
  - **UI:** Полям ввода цены и выбора валюты (в строке оффера) заданы явные стили `bg-white text-slate-900`, чтобы предотвратить отображение в "темном" виде на некоторых устройствах.
- **Файлы:** `components/AdminInterface.tsx`.

## 1.9.31
- **Admin Interface:**
  - **Верстка:** Полностью исправлена CSS-сетка таблицы заказов. Теперь используется 9 колонок с жестко заданными размерами (80px, 100px, 1fr, 60px, 120px, 110px, 110px, 90px, 30px) для идеального выравнивания.
  - **Заголовок:** Колонка "ОФФЕРЫ" теперь динамически меняет название на "СТАТУС" при переходе в архив.
  - **Дата:** Для даты выделено 90px с выравниванием по правому краю, что предотвращает "вылезание" за границы ячейки.
  - **Шеврон:** Выделена отдельная колонка в 30px для иконки раскрытия.
- **Файлы:** `components/AdminInterface.tsx`.

## 1.9.30
- **Admin Interface:**
  - **Дизайн (Rollback):** Восстановлен прежний вид раскрытого заказа. Офферы снова группируются под соответствующим товаром (Grid Layout), а не списком.
  - **Сортировка:** При клике на заголовок сортировки раскрытый заказ принудительно сворачивается, чтобы избежать визуальных ошибок.
  - **Архив:** Исправлена логика сортировки по статусу в архиве (Открыт -> Отказ -> Готово -> Куплено).
  - **UI:** Восстановлена колонка с количеством офферов.
- **Client Interface:**
  - **VIN:** Демо-заказ теперь генерирует полноценные 17-значные VIN-коды с правильными префиксами брендов.
- **Файлы:** `components/AdminInterface.tsx`, `components/ClientInterface.tsx`.

## 1.9.29
- **Client Interface:** 
  - Реализована сортировка в таблице заказов (клик по заголовкам). VIN не сортируется.
  - Статус "В ОБРАБОТКЕ" изменен на желтый цвет для привлечения внимания.
  - Улучшена валидация поля "Марка": красная рамка, если поле не заполнено (и не выбрано).
  - Обновлена логика кнопки "Демо-заказ": теперь генерирует разнообразные данные (рандомные марки, модели, года, 1-4 позиции).
- **Seller Interface:**
  - Статус "Ожидание" заменен на "Сбор офферов" (желтый цвет).
  - Реализована сортировка таблицы заказов.
- **Файлы:** `components/ClientInterface.tsx`, `components/SellerInterface.tsx`.

## 1.9.28
- **Client Interface:** 
  - VIN вынесен из первой колонки (где он был под ID) в отдельную колонку после "Модель".
  - Ширина колонки VIN установлена в 130px.
- **Seller Interface:**
  - Добавлена колонка VIN (130px) после "Модель".
  - Обеспечено отображение VIN в мобильной (карточной) версии.
- **Файлы:** `components/ClientInterface.tsx`, `components/SellerInterface.tsx`.

## 1.9.27
- **Seller Interface:** 
  - Финальное исправление выравнивания заголовков таблицы.
  - Структура контейнера заголовка приведена в точное соответствие со структурой строк данных: `border-l-4 border-transparent` + `p-3`. 
  - Это устраняет любые различия в отступах, вызванные цветным бордером слева у заказов.
- **Файлы:** `components/SellerInterface.tsx`.

## 1.9.26
- **Seller Interface:** 
  - Исправлено смещение заголовков таблицы относительно данных (эффект "лесенки").
  - Выравнивание заголовков (`p-3`, `pl-1`) синхронизировано с отступами строк (`p-3`, `border-l-4`).
  - Все текстовые элементы принудительно выровнены по левому краю.
- **Файлы:** `components/SellerInterface.tsx`.

## 1.9.25
- **Telegram (Templates):** 
  - Разделены шаблоны уведомлений об отказе:
    - Если отказался Клиент (через интерфейс): отправляется "❌ КЛИЕНТ ОТКАЗАЛСЯ".
    - Если аннулировал Админ: отправляется новый шаблон "❌ ЗАКАЗ ... был аннулирован" с указанием причины.
  - В уведомление "✅ КП СФОРМИРОВАНО" добавлен блок `💰 ИТОГО: {сумма} {валюта}` перед ссылкой.
- **Client Interface:**
  - **Footer Overflow:** Исправлен баг, когда черная плашка с ценой вылезала за пределы экрана на мобильных. Добавлен `flex-wrap` и ограничение ширины.
  - **Logout Reset:** При выходе из аккаунта теперь полностью очищается форма создания заказа (сбрасываются VIN, авто и позиции). Это предотвращает утечку данных между сессиями разных клиентов.
  - **Refusal Source:** При нажатии кнопки "Отказаться" передается флаг `source: 'CLIENT'`, чтобы бэкенд выбрал правильный шаблон уведомления.
- **Seller Interface:**
  - **Alignment:** Принудительное выравнивание по левому краю (`text-left`) для всех колонок таблицы, включая Дату и Статус, для устранения визуальной "лесенки".
- **Backend (GAS):**
  - Обновлена функция `refuse_order` для приема параметра `source`.
  - Добавлена функция `formatAdminAnnulmentMessage`.
- **Файлы:** `archive/TG_BLOCK_V2.js`, `components/ClientInterface.tsx`, `components/SellerInterface.tsx`, `components/AdminInterface.tsx`, `services/sheetService.ts`.

## 1.9.24
- **Telegram (Templates):** Улучшено форматирование уведомлений.
  - Жирное выделение цен и количества (`100$ x n шт`).
  - Уведомление "Новое предложение" содержит блок "ИТОГО" в конце списка позиций.
  - Формат заголовка ссылки на CRM стандартизирован: `{Brand} | {Model} | {Year} | {Client}`.
- **Client Interface:**
  - **Валидация Формы:** Поля "Марка" и "Название детали" подсвечиваются красным, если пусты. Кнопка отправки заблокирована до корректного заполнения.
  - **Архив:** Добавлено отображение причины отказа (красная плашка) в футере карточки заказа в архиве.
  - **Анимация:** Добавлена анимация "сгорания" (vanishing) при аннулировании заказа.
- **Seller Interface:**
  - **Авторизация:** Внедрена маска для китайских номеров `+86 1XX XXXX XXXX` (11 цифр + код страны).
  - **Валюта:** По умолчанию выбрана валюта `CNY` (Юань).
  - **Цена:** Ограничение ввода цены до 1,000,000 (при вводе большего числа значение сбрасывается на 1 млн, а не 0).
  - **UI:** Выравнивание содержимого таблицы заказов по левому краю.
- **Файлы:** `archive/TG_BLOCK_V2.js`, `components/ClientInterface.tsx`, `components/SellerInterface.tsx`.

## 1.9.23
- **Telegram (Templates):** Полный редизайн всех уведомлений (Заказ, Предложение, КП, Покупка, Отказ).
  - Ссылки на CRM теперь ведут на конкретный лид с заголовком `{Авто} | {Клиент}`.
  - Уведомление "Новое предложение" теперь содержит список позиций с ценами.
  - Уведомление "Покупка" содержит призыв "Сконвертируйте в сделку!".
  - Уведомление "Отказ" содержит финансовый расчет упущенной выгоды (ИТОГО).
- **Client Interface:** 
  - Исправлен баг дублирования заказов без VIN (теперь принудительно ставится "N/A").
  - Хедер таблицы заказов приведен к единому стилю с поставщиком.
- **Seller Interface:**
  - Переименована колонка "ORD" -> "№ заказа".
  - Выравнивание колонок по левому краю для лучшей читаемости.
  - Добавлена кнопка копирования VIN.
  - Обновлены тексты в футере карточки заказа (статусы обработки).
- **Admin Interface:**
  - **Группировка отказов:** В развернутом заказе поставщики, у которых "Нет в наличии", группируются в одну строку summary, не засоряя список.
  - **Аннулирование:** Добавлено обязательное поле "Причина отказа". Причина сохраняется и отображается клиенту.
  - **Сброс Лидера:** Повторное нажатие на кнопку "Лидер" (зеленую) снимает ранг и возвращает кнопку "Выбрать".
  - **Защита:** Блокировка утверждения КП, если нет ни одного оффера.
- **Файлы:** `archive/TG_BLOCK_V2.js`, `components/ClientInterface.tsx`, `components/SellerInterface.tsx`, `components/AdminInterface.tsx`.

## 1.9.22
- **Backend (GAS):** Исправлено форматирование заголовка автомобиля в Telegram и Google Sheets. Убраны фигурные скобки, добавлен разделитель `|`.
- **Backend (GAS):** Восстановлены цены в текстовой сводке заказа (Колонка I). Теперь формат: `✅ | Товар | Кол-во | Цена`. Это обеспечивает корректную работу калькулятора "ИТОГО".
- **Backend (GAS):** Ссылки на Битрикс24 в уведомлениях Telegram (Предложение, КП, Покупка) теперь оформлены как гиперссылки с текстом `{Авто} | {Клиент}`.
- **Backend (GAS):** В уведомление о покупке добавлен призыв ", сконвертируйте в сделку!".
- **Файлы:** `archive/TG_BLOCK_V2.js`.

## 1.9.21
- **Backend (GAS):** Полная переработка логики уведомлений в Telegram для менеджеров.
- **Backend (GAS):** Обновлен формат сводки заказа (Колонка I). Теперь отображается марка, модель и год авто, а позиции следуют списком с чекбоксами.
- **Backend (GAS):** Интеграция ссылок на Битрикс24 во все уведомления (Новый заказ, Оффер, КП, Покупка). ID лида сохраняется в JSON заказа при создании.
- **Backend (GAS):** Новое уведомление "💰 НОВОЕ ПРЕДЛОЖЕНИЕ" теперь содержит контекст: Авто, Клиент, VIN.
- **Backend (GAS):** Уведомление "✅ КП СФОРМИРОВАНО" теперь берет актуальные данные из JSON (включая AdminName и AdminPrice) и показывает только утвержденные позиции.
- **Backend (GAS):** Уведомление "🛍 КЛИЕНТ ГОТОВ КУПИТЬ" теперь автоматически считает сумму "ИТОГО" по утвержденным позициям и содержит призыв к конвертации лида.
- **Файлы:** `archive/TG_BLOCK_V2.js`.
...
- **Зачем:** Выполнение задачи Этапа 1 (Валидация, UX формы заказа, Контакты) согласно плану.
